    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
      <div className={`w-full max-w-md rounded-2xl shadow-2xl overflow-hidden border ${isDarkMode ? 'bg-slate-900 border-red-900/50' : 'bg-white border-red-200'}`}>
        <div className={`p-6 border-b ${isDarkMode ? 'border-slate-800' : 'border-slate-100'} flex items-center gap-4`}>
          ...
          <div>
            <h3 className={`text-xl font-black ${isDarkMode ? 'text-white' : 'text-slate-900'} uppercase tracking-tight`}>Atenção ao Horário</h3>
            <p className={`text-sm ${isDarkMode ? 'text-slate-400' : 'text-slate-500'} font-medium mt-1`}>
              O horário inserido ({timeStr}) já passou na data atual.
            </p>
          </div>
        </div>
        ...
        <div className="p-6 space-y-3">
            ...
            >
              ...
              <div className="text-left flex-1">
                <p className={`font-black text-[13px] uppercase tracking-wide ${isDarkMode ? 'text-amber-400' : 'text-amber-700'}`}>Manter Hoje (Atrasado)</p>
                <p className={`text-[11px] ${isDarkMode ? 'text-slate-400' : 'text-slate-500'} font-medium`}>O voo estava programado para hoje e está atrasado.</p>
              </div>
            </button>
            ...
            >
              ...
              <div className="text-left flex-1">
                <p className={`font-black text-[13px] uppercase tracking-wide ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`}>Reagendar para malha do dia seguinte</p>
                <p className={`text-[11px] ${isDarkMode ? 'text-slate-400' : 'text-slate-500'} font-medium`}>Este voo pertence à operação de amanhã.</p>
              </div>
            </button>
